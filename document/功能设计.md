...
| preview          | varchar(100)    |          | NULL   | 笔记预览                           |             | 如果预览的长度超过60字，则在末尾手动添加符号 `…`。 |
...
CREATE TABLE `note_info` (
  ...
  `preview` varchar(100) DEFAULT NULL COMMENT '笔记预览',
...
| --------- | ------------ | -------- | ------ | ------------------------------------------------------------ | ----------- | -------- |
| id        | varchar(32)  | NOT NULL |        | ID                                                           | PRIMARY KEY |          |
| type      | tinyint      |          | NULL   | 资源类型（0：用户头像，1：笔记图片，2：网站资源，3：其他）   | KEY         |          |
...
CREATE TABLE `aliyun_oss_resource` (
  `id` varchar(32) NOT NULL COMMENT 'ID',
  `type` tinyint DEFAULT NULL COMMENT '资源类型（0：用户头像，1：笔记图片，2：网站资源，3：其他）',
